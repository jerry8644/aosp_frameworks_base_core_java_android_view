...
public final class ViewRootImpl implements ViewParent,
    ...
    int mSeq;
    private ScrollMonitorManager mScrollMonitorManager;
    ...
    final Rect mOrigSurfaceInsets = new Rect();
    ...
    public ViewRootImpl(Context context, Display display) {
        ...
        mScrollMonitorManager = ScrollMonitorManager.getScrollMonitorManager();
    ...
    public void setView(View view, WindowManager.LayoutParams attrs, View panelParentView) {
        synchronized (this) {
            if (mView == null) {
                ...
                mOrigSurfaceInsets.set(mWindowAttributes.surfaceInsets);
    ...
    private int frameTimeThershold = 35;
    ...
    private void performTraversals() {
        ...
        if (mFirst || windowShouldResize || insetsChanged ||
            ...
            if(mFirst) {
                mOrigWidth = frame.width();
                mOrigHeight = frame.height();
            }
    ...
    private boolean draw(boolean fullRedrawNeeded) {
        ...
        if (isResizeEnabled) {
            mAttachInfo.mViewRootImpl.mChoreographer.mFrameInfo.addFlags(FrameInfo.FLAG_SCALE_NEEDED);
        }

        if (mAttachInfo.mViewScrollChanged) {
            if(isResizeEnabled) {
                mHandler.removeMessages(MSG_DISPATCH_MODE_CHANGED);
                mHandler.sendMessageDelayed(mHandler.obtainMessage(MSG_DISPATCH_MODE_CHANGED), frameTimeThershold);
            }
            mAttachInfo.mViewScrollChanged = false;
            mAttachInfo.mTreeObserver.dispatchOnScrollChanged();
        }

        //Log.i(mTag, "scrollX=" + mView.getScrollX() + " scrollY=" + mView.getScrollY());
        ...
        if (!dirty.isEmpty() || mIsAnimating || accessibilityFocusDirty) {
            if (mAttachInfo.mThreadedRenderer != null && mAttachInfo.mThreadedRenderer.isEnabled()) {
                ...
                if (!isResizeHandled) {                  
                    RenderNode HWRenderNode = mAttachInfo.mThreadedRenderer.getRootNode();
                    if (firstScale) { 
                        HWRenderNode.setPivotX(0);
                        HWRenderNode.setPivotY(0);
                        firstScale = false;
                    }
                    //每次draw執行後會跑進來可以設定if/
                    HWRenderNode.setScaleX(scaleFactor);
                    HWRenderNode.setScaleY(scaleFactor);
                    Log.i(mTag, "Scalefactor_draw1="+scaleFactor);
                    HWRenderNode.setLeftTopRightBottom(0, 0, (int) (mView.getMeasuredWidth() * scaleFactor + 0.5f), (int) (mView.getMeasuredHeight() * scaleFactor + 0.5f));                    
                    Log.d(mTag, "RenderNode=" + HWRenderNode + " w=" + HWRenderNode.getWidth() + " h=" + HWRenderNode.getHeight() + " pivotX=" + HWRenderNode.getPivotX() + " pivotY=" + HWRenderNode.getPivotY() +
                    "\nl=" + HWRenderNode.getLeft() + " t=" + HWRenderNode.getTop() + " r=" + HWRenderNode.getRight() + " b=" + HWRenderNode.getBottom() +
                    " scaleX=" + HWRenderNode.getScaleX() + " scaleY=" + HWRenderNode.getScaleY());
                ...
                if (invalidateRoot) {
                    Log.d(mTag, "invalidateRoot is called");
    ...
    private static final int MSG_DISPATCH_RESOLUTION_CHANGED = 34;
    private static final int MSG_DISPATCH_OUTPUT_INFO = 35;
    private static final int MSG_DISPATCH_FRAME_RATE_CHANGED = 36;
    private static final int MSG_DISPATCH_MODE_CHANGED = 37;
    ...
    final class ViewRootHandler extends Handler {
        ...
        public String getMessageName(Message message) {
            switch (message.what) {
                ...
                case MSG_DISPATCH_RESOLUTION_CHANGED:
                    return "MSG_DISPATCH_RESOLUTION_CHANGED";
                case MSG_DISPATCH_OUTPUT_INFO:
                    return "MSG_DISPATCH_OUTPUT_INFO";
                case MSG_DISPATCH_FRAME_RATE_CHANGED:
                    return "MSG_DISPATCH_FRAME_RATE_CHANGED";
                case MSG_DISPATCH_MODE_CHANGED:
                    return "MSG_DISPATCH_MODE_CHANGED";
            }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_DISPATCH_RESOLUTION_CHANGED: {
                    ...
                    SomeArgs args = (SomeArgs) msg.obj;
                    isResizeEnabled = (boolean) args.arg1;                     
                    scaleFactor = (float) args.arg2;
                    scaleFactor = (float) (((int)(scaleFactor * 1000)) / 1000.0f);
                    frameTimeThershold = (int) (((int) args.arg3) * 1.5f);
                    //Log.i(mTag, "scaleFactor=" + scaleFactor + " isResizeEnabled=" + isResizeEnabled + " isResizeHandled=" + isResizeHandled + " frameTimeThershold=" + frameTimeThershold);
                    args.recycle();
                    //invalidate();
                } break;
                case MSG_DISPATCH_OUTPUT_INFO:
                    //mView.mRenderNode.output();
                    int config = mSurfaceControl.getActiveConfig(SurfaceControl.getInternalDisplayToken());
                    Display.Mode[] modes = mDisplay.getSupportedModes();
                    FileDescriptor fd = (FileDescriptor) msg.obj;
                    int maxFlingVelocity = mViewConfiguration.getScaledMaximumFlingVelocity(); 
                    int minFlingVelocity = mViewConfiguration.getScaledMinimumFlingVelocity();
                    try {
                        outputStream = new FileOutputStream(fd);
                        pw = new FastPrintWriter(outputStream);
                        pw.println("mContext=" + mContext);
                        pw.println("maxFlingVelocity=" + maxFlingVelocity + " minFlingVelocity=" + minFlingVelocity);
                        pw.println("mView.getContext()=" + mView.getContext());
                        pw.println("active config: " + config);
                    ...
                    break;
                case MSG_DISPATCH_FRAME_RATE_CHANGED:
                    mFPSDivisor = msg.arg1;
                    mChoreographer.setFPSDivisor(mFPSDivisor);
                    break;
                case MSG_DISPATCH_MODE_CHANGED: {
                    isResizeHandled = false;
                    if (msg.obj == null) {
                        scaleFactor = 1.0f; // scoll stop change to default
                        isResizeEnabled = false;
                        
                        
                        //mFPSDivisor = 1;
                        //mChoreographer.setFPSDivisor(mFPSDivisor);
                        mScrollMonitorManager.notifyScrollStopped();
                    ...
                    SomeArgs args = (SomeArgs) msg.obj;
                    scaleFactor = (float) args.arg1;                     
                    scaleFactor = (float) (((int)(scaleFactor * 1000)) / 1000.0f);
                    frameTimeThershold = (int) (((int) args.arg2) * 2.0f + 2);
                    mFPSDivisor = (int) args.arg3;
                    mChoreographer.setFPSDivisor(mFPSDivisor);
                    args.recycle();
                    Log.d(mTag, "scaleFactor=" + scaleFactor + " isResizeEnabled=" + isResizeEnabled + " isResizeHandled=" + isResizeHandled + " frameTimeThershold=" + frameTimeThershold);
                    invalidate();
                } break;
    ...
    private void dispatchResolutionChanged(boolean isResizeEnabled, float scaleFactor, int frameTime) {
        SomeArgs args = SomeArgs.obtain();
        args.arg1 = isResizeEnabled;
        args.arg2 = scaleFactor;
        args.arg3 = frameTime;
        Message msg = mHandler.obtainMessage(MSG_DISPATCH_RESOLUTION_CHANGED, args);
        mHandler.sendMessageAtFrontOfQueue(msg);
    }

    private void dispatchOutputInfo(FileDescriptor fd) {
        Message msg = mHandler.obtainMessage(MSG_DISPATCH_OUTPUT_INFO, fd);
        mHandler.sendMessage(msg);
    }

    private void dispatchFrameRateChanged(int divisor) {
        Message msg = mHandler.obtainMessage(MSG_DISPATCH_FRAME_RATE_CHANGED, divisor, 0);
        mHandler.sendMessage(msg);
    }

    private void dispatchModeChanged(float scaleFactor, int frameTime, int divisor) {
        SomeArgs args = SomeArgs.obtain();
        args.arg1 = scaleFactor;
        args.arg2 = frameTime;
        args.arg3 = divisor;
        Message msg = mHandler.obtainMessage(MSG_DISPATCH_MODE_CHANGED, args);
        mHandler.sendMessageAtFrontOfQueue(msg);
        RenderNode HWRenderNode = mAttachInfo.mThreadedRenderer.getRootNode();
        HWRenderNode.setScaleX(scaleFactor);
        HWRenderNode.setScaleY(scaleFactor);
    ...
    static class W extends IWindow.Stub {
        ...
        @Override
        public void dispatchResolutionChanged(boolean isResizeEnabled, float scaleFactor, int frameTime) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchResolutionChanged(isResizeEnabled, scaleFactor, frameTime);
            }
        }

        @Override
        public void outputInfo(FileDescriptor fd) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchOutputInfo(fd);
            }
        }

        @Override
        public void setFrameRate(int divisor) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchFrameRateChanged(divisor);
            }
        }

        @Override
        public void dispatchModeChanged(float scaleFactor, int frameTime, int divisor) {
            final ViewRootImpl viewAncestor = mViewAncestor.get();
            if (viewAncestor != null) {
                viewAncestor.dispatchModeChanged(scaleFactor, frameTime, divisor);
            }
        }
